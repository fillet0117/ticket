...
/*--------------------有直達的票----------------------*/
            ...
            elseif($cost[$start][$end] < $count){
                ...
                for($w = $for_start; $w < $for_end; $w++){
                    ...
                    if($cost[$w][$for_end] != 0){
                        ...
                        $min = min($min,$num);
                        $prior_copy[$u] = $prior;
                        for($e = 0; $e < count($prior); $e++){
                            if($prior_copy[$u][$e] != $start){
                                $set2++;
                            }
                        }
                        ...
                        ?>
                                <td><?php echo "<form id=form$u name=form$u Action=tickets2.php method=get>";
                                            echo "<input type=hidden name=item value=$m>";
        ...
        //echo $check_left,$check_have2,$check_arri;
        /*if($check_left != 1 && $check_have2 !=0){  //轉站的票數是夠
            if($check_arri == 0){   //直達的票夠
               // echo '<form method = get action = check.php>';
                echo "<input type = hidden name = start_pass value=$start_user>";
                echo "<input type = hidden name = end_pass value=$end_user>";
                echo "<input type = hidden name = count_pass value=$count>";
                echo "<input type=hidden name=item value=$m>";
            ...
            elseif($check_arri == 1){ //有直達的票,也有轉站的票
                echo '<form method = get action = check.php>';
                echo "<input type = hidden name =start_pass value=$start_user>";
                echo "<input type = hidden name = end_pass value=$end_user>";
                echo "<input type = hidden name = count_pass value=$count>";
